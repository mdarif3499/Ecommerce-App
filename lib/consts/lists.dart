import 'package:ecommerce_app/consts/consts.dart';

const socialIconList = [icFacebookLogo, icGoogleLogo, icTwitterLogo];
const sliderList = [imgSlider1,imgSlider2,imgSlider3,imgSlider4];
const secondSliderList = [imgSs1,imgSs2,imgSs3,imgSs4];

const featuredImg1 = [imgS1,imgS2,imgS3];
const featuredImg2 = [imgS4,imgS5,imgS6];
const featuredTitle1 = [womenDress,girlsDress,girlswatches];
const featuredTitle2 = [boysGlasses,mobilePhone,tShirts];
const categorieesList = [womenDress,menClothingFashoin,compAccess,automobile,kidtoys,sports,jewelery,cellPhone,furniture];
const categorisImage = [imgS1,imgFc1,imgFc2,imgFc3,imgFc4,imgFc5,imgS4,imgFc6,imgFc9];
const itemDetailsButtonList = [video,reviews,privacyPolicy,returnPolicy,supportPolicy,];
const prifileButtonList = [order,wishlist,message];
const prifileIconList = [icOrders,icFav,icMessages];
const paymentMethodImg = [imgPaypal,imgStripe,imgCod];
const paymentMethod = [paypal,stripe,cod];